 ...
 * @param senderMemberId - 메시지를 보낸 사용자의 멤버 ID
 * @param content - 수정된 메시지 내용
 * @returns 수정된 메시지에 대한 서버 응답 데이터
 * @throws 서버 요청 실패 시 예외 발생
 */
export const editThreadMesaage = async (threadId: number, senderMemberId: number, content: string) => {
  try {
 ...
 * @throws 서버 요청 실패 시 예외 발생
 */
export const deleteThreadMesaage = async (threadId: number, senderMemberId: number) => {
  try {
 ...
 * @param params.senderMemberId - 답글을 작성한 사용자의 멤버 ID
